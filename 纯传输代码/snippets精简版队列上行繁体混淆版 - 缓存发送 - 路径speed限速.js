...
const 手動資料管線 = async (可讀流, 可寫通道, 關閉連線, 管線速度) => {
    const 啟用限速 = 管線速度 !== undefined, 管線緩衝區大小 = 啟用限速 ? 512 * 1024 : 緩衝區大小, 安全緩衝區大小 = 管線緩衝區大小 - 最大區塊長度, 快速刷新偏移量 = 最大區塊長度 << 1;
    let 管線刷新時間 = 刷新時間, 管線啟動閾值 = 啟動閾值;
    if (啟用限速) {
        const 數值 = parseFloat(管線速度);
        if (數值 > 0) {
            管線啟動閾值 = 數值 * 1048576;
            管線刷新時間 = Math.max(1, 管線緩衝區大小 * 1000 / 管線啟動閾值);
        }
    }
    let 緩衝區 = new ArrayBuffer(管線緩衝區大小), 備用緩衝區 = new ArrayBuffer(最大區塊長度), 緩衝區視圖 = new Uint8Array(緩衝區);
    ...
    const 刷新輸出 = () => {
        ...
        if (偏移量 > 0 && !已關閉) {
            偏移量 > 安全緩衝區大小
                ? (可寫通道.send(緩衝區視圖.subarray(0, 偏移量)), 緩衝區 = new ArrayBuffer(管線緩衝區大小), 緩衝區視圖 = new Uint8Array(緩衝區))
        ...
        while (true) {
            ...
            if (需要刷新 || 區塊長度 < 2048) {
                ...
                if (快速刷新 || 區塊長度 < 28672) {
                    if (!啟用限速) 總位元組 = 0;
                    時間 = 2;
                } else if (總位元組 > 管線啟動閾值) 時間 = 管線刷新時間;
                計時器識別 ||= setTimeout(刷新輸出, 時間), 保護刷新 = 區塊長度 < 最大區塊長度;
                偏移量 > 安全緩衝區大小 && (總位元組 > 管線啟動閾值 ? await new Promise(結果值 => 恢復函式 = 結果值) : 刷新輸出());
            }
        }
...
const 處理網頁套接字連線 = async (網頁套接字連線, 請求) => {
    ...
    const 關閉連線 = () => {
        ...
        try {網頁套接字連線.close(1011, 'WebSocket is closed')} catch {}
    };
    const 處理 = 資料區塊 => {
        try {
            ...
            return (async () => {
                ...
                const 傳輸控制結果 = await 建立傳輸控制連線(已解析請求, 請求);
                if (!傳輸控制結果) return 關閉連線();
                傳輸控制插槽 = 傳輸控制結果.連線插槽;
