...
const 手動資料管線 = async (可讀流, 可寫通道, 關閉連線, 速度) => {
    ...
    if (啟用限速) {
        ...
        for (let 大小 = 262144; 大小 <= 524288; 大小 += 65536) {
            ...
            if (絕對差值 < 最佳差值 || (絕對差值 === 最佳差值 && 時間毫秒 < 最佳時間)) 最佳大小 = 大小, 最佳時間 = 時間毫秒, 最佳差值 = 絕對差值;
        }
        管線緩衝區大小 = 最佳大小, 管線刷新時間 = 最佳時間;
    }
    const 安全緩衝區大小 = 管線緩衝區大小 - 最大區塊長度, 快速刷新偏移量 = 最大區塊長度 << 1;
    let 緩衝區視圖 = new Uint8Array(管線緩衝區大小), 備用緩衝區 = new ArrayBuffer(最大區塊長度);
    ...
    const 刷新輸出 = () => {
        ...
        if (偏移量 > 0) {
            偏移量 > 安全緩衝區大小
                ? (可寫通道.send(緩衝區視圖.subarray(0, 偏移量)), 緩衝區視圖 = new Uint8Array(管線緩衝區大小))
